...
async function translateMixedSegments({
  ...
  for (const seg of rawSegments) {
    ...
    const segmentRetrieval = {
      ...
      phraseMatches: retrievePhraseMemory(trimmed, eventMode, activeTopic),
...
function getActiveTopicTerms(activeTopic = null) {
  if (!activeTopic?.cn) return [];

  const terms = new Set([activeTopic.cn, activeTopic.en || '']);
  const entity = (sacredEntities || []).find((x) => x?.cn === activeTopic.cn);

  for (const alias of entity?.aliases || []) terms.add(alias);
  for (const mishear of entity?.mishears || []) terms.add(mishear);
  for (const related of entity?.related || []) terms.add(related);

  return Array.from(terms).filter(Boolean);
}

function scorePhraseCandidate({ text, normalizedText, row, eventMode = 'Dharma Talk', activeTopic = null }) {
  const candidateCn = row?.cn || '';
  if (!candidateCn) return 0;

  const normalizedCandidate = normalizeClusterText(candidateCn);
  let score = 0;

  if (text === candidateCn) score += 120;
  if (text.includes(candidateCn)) score += candidateCn.length * 2;
  if (normalizedText === normalizedCandidate && normalizedText) score += 40;
  if (normalizedText && normalizedCandidate && normalizedText.includes(normalizedCandidate)) {
    score += Math.max(8, Math.floor(normalizedCandidate.length * 1.5));
  }
  ...
  const modeMatches = []
    .concat(row?.event_modes || [])
    .concat(row?.event_mode || [])
    .concat(row?.eventMode || [])
    .filter(Boolean)
    .map((x) => String(x).toLowerCase());

  if (modeMatches.some((m) => String(eventMode).toLowerCase().includes(m) || m.includes(String(eventMode).toLowerCase()))) {
    score += 3;
  }

  for (const correction of correctionMemory || []) {
    const heard = normalizeClusterText(correction?.heard || '');
    const intended = normalizeClusterText(correction?.intendedChinese || correction?.corrected || '');
    const correctedEn = normalizeClusterText(correction?.correctedEnglish || '');

    if (!heard && !intended) continue;

    if (heard && normalizedText.includes(heard)) {
      if (intended && normalizedCandidate.includes(intended)) score += 10;
      if (correctedEn && normalizeClusterText(row?.en || '').includes(correctedEn)) score += 6;
    }
  }

  const topicTerms = getActiveTopicTerms(activeTopic);
  if (topicTerms.length > 0) {
    ...
    for (const term of topicTerms) {
      if (!term) continue;
      if (candidateCn.includes(term) || String(row?.en || '').includes(term)) score += 10;
      if (relatedTerms.some((x) => x.includes(term) || term.includes(x))) score += 4;
    }
  }
...
function retrievePhraseMemory(text, eventMode = 'Dharma Talk', activeTopic = null) {
  const pools = [...phraseMemory, ...generatedPhrases];
  const results = [];
  const normalizedText = normalizeClusterText(text);

  for (const row of pools) {
    const score = scorePhraseCandidate({
      text,
      normalizedText,
      row,
      eventMode,
...
function findPhraseMatch(text, mode = 'final', activeTopic = null, eventMode = 'Dharma Talk') {
  const matches = retrievePhraseMemory(text, eventMode, activeTopic);
  if (!matches.length) return null;

  const best = matches[0];
  if (!best?.en) return null;

  if ((best._score || 0) >= 40) return best;
  if (mode === 'interim' && (best._score || 0) >= 18) return best;
  return null;
}
...
function buildTranslationMeta({
  ...
  const phraseMatch = findPhraseMatch(normalizedCn, mode, activeTopic, retrieval?.eventMode || 'Dharma Talk');
...
async function translateWithDeepSeek(
  ...
  const phraseMatch = findPhraseMatch(text, mode, activeTopic, eventMode);
...
app.post('/api/session/:id/line', async (req, res) => {
  ...
  const retrieval = {
    ...
    phraseMatches: retrievePhraseMemory(normalizedCn, session.eventMode, getActiveTopicContext(session.brainState)),
  ...
  retrieval.eventMode = session.eventMode;
...
app.post('/api/translate-interim', async (req, res) => {
  ...
  const retrieval = {
    ...
    phraseMatches: retrievePhraseMemory(normalizedCn, eventMode, getActiveTopicContext(session.brainState)),
      ...
      try {
        ...
        if (data.is_final) {
          const retrieval = {
            ...
            phraseMatches: retrievePhraseMemory(normalizedCn, activeSession.eventMode, getActiveTopicContext(activeSession.brainState)),
          ...
          retrieval.eventMode = activeSession.eventMode;
